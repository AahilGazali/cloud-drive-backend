...
export const createFolder = async (userId, payload) => {
  ...
  if (parentId) {
    const { rows: parentRows } = await query(
      "SELECT id, owner_id FROM folders WHERE id=$1 AND is_deleted=false",
      [parentId]
    );
    if (!parentRows.length) throw new Error("Parent folder not found");
    if (parentRows[0].owner_id !== userId) throw new Error("Forbidden");
  }
...
export const listChildren = async (userId, folderId = null) => {
  const { rows } = await query(
    `SELECT f.*, COALESCE(s.role, 'owner') AS access_role
     ...
     WHERE f.parent_id ${folderId ? "= $2" : "IS NULL"} AND f.is_deleted=false AND (f.owner_id=$1 OR s.role IN ($3,$4))`,
    folderId
...
export const moveFolder = async (userId, payload) => {
  ...
  const { rows } = await query(
    "UPDATE folders SET parent_id=$1 WHERE id=$2 AND owner_id=$3 RETURNING *",
